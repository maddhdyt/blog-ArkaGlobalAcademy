            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50">
                        ...
                        <th class="py-4 px-6 text-xs font-bold text-slate-700 uppercase tracking-wider border-b border-slate-100">Role</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($users as $user)
                        <tr class="hover:bg-slate-50 transition-colors group">
                            <td class="py-4 px-6 text-sm font-bold text-slate-700">{{ $user->name }}</td>
                            <td class="py-4 px-6 text-sm font-semibold text-slate-700">{{ $user->email }}</td>
                                ...
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    @if (auth()->id() !== $user->id)
                                        <form action="{{ route('admin.users.destroy', $user) }}" method="POST" class="inline-block"
                                            onsubmit="confirmDelete(event, this, 'Hapus Pengguna?', 'Pengguna yang dihapus tidak dapat dikembalikan.')">
                                            @csrf
                                            @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="py-8 text-center text-slate-700 font-semibold">Belum ada user.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
