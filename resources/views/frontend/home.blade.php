                    ...
                    @forelse ($featureGridPosts as $post)
                        ...
                        <article class="flex flex-col group bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-md transition-shadow">
                            ...
                            <div class="flex flex-1 flex-col p-6 sm:p-8">
                                ...
                                <p class="text-[14px] sm:text-[15px] text-slate-600 line-clamp-3 leading-relaxed mb-8">
                                    {{ \Illuminate\Support\Str::limit($post->excerpt ?: strip_tags($post->content ?? ''), 120) }}
                                </p>
                                <div class="mt-auto inline-flex items-center self-start gap-2 text-xs text-slate-500 bg-orange-50 px-2 py-0.5 font-mono tracking-wide">
                ...
                <div class="bg-white p-8 border border-slate-200 shadow-sm rounded-2xl">
                    ...
                    <form action="{{ route('newsletter.subscribe') }}" method="POST" class="mt-6 space-y-4">
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" placeholder="Nama Lengkap"
                            class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:border-orange-500 focus:bg-white focus:ring-4 focus:ring-orange-500/20 focus:outline-none transition-colors text-sm @error('name') border-red-500 @enderror">
                        <input type="email" name="email" value="{{ old('email') }}" placeholder="Alamat Email"
                            class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:border-orange-500 focus:bg-white focus:ring-4 focus:ring-orange-500/20 focus:outline-none transition-colors text-sm @error('email') border-red-500 @enderror" required>
                        @error('email')
                    ...
                    <div id="newsletter-toast"
                        class="pointer-events-none fixed bottom-6 left-1/2 -translate-x-1/2 transform bg-slate-900 text-white px-6 py-4 shadow-2xl opacity-0 translate-y-3 transition duration-300 font-mono text-sm tracking-wide"
                        style="z-index:60;">
                        ...
                        @forelse ($trendingPosts as $post)
                            ...
                            <article class="flex gap-5 group items-start pb-4 border-b border-gray-100 last:border-0 last:pb-0">
                                ...
                                <div class="flex flex-1 flex-col">
                                    ...
                                    <h4 class="text-[17px] font-normal text-faux-medium leading-snug text-slate-900 font-heading group-hover:text-brand-primary transition">
                                        <a href="{{ route('posts.show', $post->slug) }}">{{ $post->title }}</a>
                                    </h4>
                                    <p class="mt-2 text-[13px] text-slate-600 line-clamp-2 leading-relaxed">
